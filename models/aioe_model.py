...
@register_model("aioe_transformer")
class AIOEModel(TransformerModelBase):
    """
    ...
    def forward_encoder(self, src_tokens, src_lengths, src_position_ids, src_segment_ids, return_all_hiddens=True):
        ...
        encoder_hidden_states = encoder_out["encoder_out"][0].transpose(0,1)
           
        # gather mask tokens
        src_tokens_mask = src_tokens.eq(self.encoder.dictionary.index("<mask>"))
        mask_hidden_states = encoder_hidden_states[src_tokens_mask]
        mask_logits = self.decoder.output_layer(mask_hidden_states)
        return mask_logits
    ...
    def forward(
        ...
        if self.training:
            if not self.cfg.no_ar_task:
                ...
                return mask_logits, decoder_out
            else:
...
# architectures
@register_model_architecture("aioe_transformer", "aioe_transformer")
def base_architecture(args):
    ...
    args.replace_positon_segment = getattr(args, "replace_positon_segment", True)