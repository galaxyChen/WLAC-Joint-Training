...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                logger.info("We have missing keys: {}".format(missing_keys))
                logger.info("We have unexpected keys: {}".format(unexpected_keys))
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        def splice_sample(sample, sample_size):
            # sample: dict
            if sample_size == 0:
                return
            for k, v in sample.items():
                if isinstance(v, torch.Tensor):
                    if k == "ntokens":
                        ...
                        sample[k] = v[:sample_size]
                elif isinstance(v, dict):
                    splice_sample(sample[k], sample_size)
            ...
            while batch_size > 0:
                try:
                    ...
                    if sample_discount != 1:
                        logger.info("Recover from OOM, sample size: {}, set discount to 1".format(sample["id"].size(0)))
                        sample_discount = 1
                    break
                    ...
                    if "out of memory" in str(e):
                        ...
                        sample_discount /= 2
                        batch_size = int(batch_size * sample_discount)
                        logger.info("Adjusting sample size, OOM sample size: {}, set discount to {}, set sample size to {}".format(sample["id"].size(0), sample_discount, batch_size))
                        splice_sample(sample, batch_size)
