...
def infer_init_method(cfg: DistributedTrainingConfig, force_distributed=False):
    logger.info("Infering distributed init method")
    # logger.info("Setting envrionment variables for distributed training")
    # if "CHIEF_IP" in os.environ:
    #     os.environ["MASTER_ADDR"] = os.environ["CHIEF_IP"]
    # if "HOST_GPU_NUM" in os.environ:
    #     os.environ["WORLD_SIZE"] = os.environ["HOST_GPU_NUM"]
    # if "INDEX" in os.environ:
    #     os.environ["RANK"] = os.environ["INDEX"]
    ...
    logger.info("Environment variables: {}".format(os.environ))
    if all(
        key in os.environ
        for key in ["MASTER_ADDR", "MASTER_PORT", "WORLD_SIZE", "RANK"]
    ):
        # support torch.distributed.launch
        logger.info("_infer_torch_distributed_launch_init")
...
def distributed_init(cfg: FairseqConfig):
    ...
    if not cfg.common.tpu:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            logger.info("NCCL communicator initialized")
