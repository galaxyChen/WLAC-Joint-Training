...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.common.model_parallel_size == 1:
        logger.info("Task is {}".format(cfg.task))
        trainer = Trainer(cfg, task, model, criterion, quantizer)
        # if cfg.task._name.startswith("em_wlac"):
        #     trainer = EMWlacTrainer(cfg, task, model, criterion, quantizer)
        # else:
        #     trainer = Trainer(cfg, task, model, criterion, quantizer)
    else:
    ...
    task.epoch_idx = epoch_itr.epoch    # ! Important Modification
    if cfg.common.tpu:
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        task.epoch_idx = task.epoch_idx + 1    # ! Important Modification
    train_meter.stop()
