...
from data.aioe_bpe_dataset import AIOEBPEDataset
from criterions.aioe_bpe_loss import AIOEBPELoss
...
class BPE:
    def __init__(self, src_code, src_vocab, tgt_code, tgt_vocab) -> None:
    ...
    def apply_src(self, lines):
        if self.src_bpe is None:
            self.src_bpe = fastBPE.fastBPE(self.src_code, self.src_vocab)
        return self.src_bpe.apply(lines)

    def apply_tgt(self, lines):
        if self.tgt_bpe is None:
            self.tgt_bpe = fastBPE.fastBPE(self.tgt_code, self.tgt_vocab)
        return self.tgt_bpe.apply(lines)
    
    def decode_src(self, lines):
        return [(line + " ").replace(self.bpe_symbols, "").rstrip() for line in lines]
    
    def decode_tgt(self, lines):
        return [(line + " ").replace(self.bpe_symbols, "").rstrip() for line in lines]
...
def load_langpair_dataset(
    ...
    logger.info(f"This dataset is for aioe_bpe task.")
    return AIOEBPEDataset(
        src_dataset,
        src_dataset.sizes,
        src_dict,
        tgt_dataset,
...
@dataclass
class AIOEBPEConfig(FairseqDataclass):
...
@register_task("aioe_bpe", dataclass=AIOEBPEConfig)
class AIOEBPETask(FairseqTask):
    """
    ...
    def __init__(self, cfg: AIOEBPEConfig, src_dict, tgt_dict):
        super().__init__(cfg)
        ...
        if self.cfg.target_lang == "zh":
            self.symbol2pinyin = True

    @classmethod
    def setup_task(cls, cfg: AIOEBPEConfig, **kwargs):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        # build bpe
        if self.cfg.bpe_path != "":
            bpe = BPE("{}/codes.{}".format(self.cfg.bpe_path, src), "{}/vocab.{}".format(self.cfg.bpe_path, src),
                      "{}/codes.{}".format(self.cfg.bpe_path, tgt), "{}/vocab.{}".format(self.cfg.bpe_path, tgt))
        else:
            bpe = None
