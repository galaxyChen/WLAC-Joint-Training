...
from tasks.aioe_bpe import AIOEBPETask
from models.aioe_bpe_model import AIOEBPEModel
logger = logging.getLogger("Generator")

class GenerationPipeline:
    def __init__(self, model_path=None, models=None, cfg=None, task=None, beam_size=5, use_cuda=True) -> None:
    ...
    def postprocess(self, hypo_tokens):
        hypo_str = self.tgt_dict.string(hypo_tokens, bpe_symbol="@@ ")
        return hypo_str

    def generate(self, samples: str, ids=None):
        ...
        if ids is None:
            ids = 0
        batch = self.form_batch(samples, ids)
        bsz = batch["src_tokens"].size(0)
        src_tokens = batch["src_tokens"]
        src_lengths = batch["src_lengths"]
        segment_ids = batch["src_segment_ids"]
        position_ids = batch["src_position_ids"]
        if use_cuda:
        ...
        for id_, src_tokens, hypos, info in sorted(results, key=lambda x: x[0]):
            ...
            for hypo in hypos[: min(len(hypos), cfg.generation.nbest)]:
                hypo_str = self.postprocess(hypo_tokens=hypo["tokens"].int().cpu())
                topk.append(hypo_str)
            hypo_res.append(topk)
        return hypo_res
